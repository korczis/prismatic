defmodule Prismatic.Event.Backend.CircuitBreaker do
  ...
  @doc """
  Get or create a circuit breaker for a backend type.

  If a circuit breaker already exists for the backend type, returns it.
  ...
  - `{:ok, pid}` - Circuit breaker process
  - `{:error, reason}` - Failed to get or create

  ## Examples

      iex> {:ok, pid} = Prismatic.Event.Backend.CircuitBreaker.get_or_create(:test)
      iex> is_pid(pid)
      true
  """
  @spec get_or_create_circuit_breaker(Protocol.backend_type()) :: {:ok, pid()} | {:error, term()}
  def get_or_create_circuit_breaker(backend_type) do
